...
def create_CNN_model(input_shape=(416,416,1)):
...
def create_morph_model_type2(input_shape=(512, 512, 1)):
    ...
    z=Conv2D(1,(1,1),activation="sigmoid")(z)
...
def create_morph_path12_model(input_shape=(512, 512, 1)):
    I=Input(shape=input_shape)
    z1=I
    z2=I
    for i in range(2):
    ...
    z=Concatenate()([z1,z2])
    z=Conv2D(1,(1,1),activation="sigmoid",padding="same")(z)
    model=Model(inputs=[I],outputs=[z])
    #model.compile(loss=DSSIMObjective(kernel_size=100), optimizer="RMSprop",metrics=['mse'])
    return model
...
def path12_old(input_shape=(512, 512, 1)):
    I = Input(shape=input_shape)
    z1 = I
    z2 = I
    for i in range(2):
...
def path1_old(input_shape=(416, 416, 1)):
    I = Input(shape=input_shape)
...
def path2_old(input_shape=(416, 416, 1)):
    I = Input(shape=input_shape)
...
#single path of dialtion and erosion lastly linear combination 
def model_new(input_shape=(416, 416, 1)):
...
"""
def path1_old(input_shape=(512, 512, 1)):
    I = Input(shape=input_shape)
    # I=Input(shape=(None,None,1))
    #z1=Dilation2D(4, (8,8),padding="same",strides=(1,1))(I)
    z1 = I
...
def path2_old(input_shape=(512, 512, 1)):
    I = Input(shape=input_shape)
...
def loss_all(y_pred,y_true):
    ...
    loss_ssim=(1-tf.image.ssim_multiscale(y_true, y_pred,filter_size=10,max_val=1))/2.0
    #loss_ssim=DSSIMObjective(kernel_size=100)(y_true,y_pred)   #+ K.mean(K.abs(y_true-y_pred))
    loss_mse=K.mean(K.square(y_true-y_pred))
    loss_mae=K.mean(K.abs(y_true-y_pred))

    loss=loss_mae+loss_ssim
    return loss
